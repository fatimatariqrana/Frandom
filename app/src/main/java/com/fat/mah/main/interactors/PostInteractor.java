...
import com.fat.mah.listeners.OnPostCreatedListener;
import com.fat.mah.listeners.OnPostListChangedListener;
...
public class PostInteractor {
    ...
    public String generatePostId() {   // generate key
    ...
    public void incrementWatchersCount(String postId) {  // increment views
    ...
    public void getPostListByUser(final OnDataChangedListener<Post> onDataChangedListener, String userId) {
        DatabaseReference databaseReference = databaseHelper.getDatabaseReference().child(DatabaseHelper.POSTS_DB_KEY);
        Query postsQuery;
        postsQuery = databaseReference.orderByChild("authorId").equalTo(userId);  // user's profile post list
    ...
    private PostListResult parsePostList(Map<String, Object> objectMap) {
        ...
        if (objectMap != null) {
            ...
            Collections.sort(list, (lhs, rhs) -> ((Long) rhs.getCreatedDate()).compareTo(lhs.getCreatedDate()));  // sorted on the basis of date
