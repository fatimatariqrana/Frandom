...
package com.fat.mah.main.interactors;
...
import com.fat.mah.ApplicationHelper;
import com.fat.mah.DatabaseHelper;
import com.fat.mah.listeners.OnDataChangedListener;
import com.fat.mah.listeners.OnTaskCompleteListener;
...
public class CommentInteractor {
    ...
    public void createComment(String commentText, final String postId, final OnTaskCompleteListener onTaskCompleteListener) {
        try {
            String authorId = FirebaseAuth.getInstance().getCurrentUser().getUid();   // user to comment
            ...
            String commentId = mCommentsReference.push().getKey();  // comment id
            ...
            mCommentsReference.child(commentId).setValue(comment, new DatabaseReference.CompletionListener() {
                ...
                private void incrementCommentsCount(String postId) {
                    DatabaseReference postRef = databaseHelper.getDatabaseReference().child(DatabaseHelper.POSTS_DB_KEY + "/" + postId + "/commentsCount");   // this is posts table
                        ...
                        public void onComplete(DatabaseError databaseError, boolean b, DataSnapshot dataSnapshot) {
                           // LogUtil.logInfo(TAG, "Updating comments count transaction is completed.");
                            if (onTaskCompleteListener != null) {
                                onTaskCompleteListener.onTaskComplete(true);  // completee
                            }
                        }
                    });
                }
    ...
    public void updateComment(String commentId, String commentText, String postId, final OnTaskCompleteListener onTaskCompleteListener) {
        DatabaseReference mCommentReference = databaseHelper.getDatabaseReference().child(DatabaseHelper.POST_COMMENTS_DB_KEY).child(postId).child(commentId).child("text");  // access comment text
    ...
    public ValueEventListener getCommentsList(String postId, final OnDataChangedListener<Comment> onDataChangedListener) {  // display comments list
        ...
        ValueEventListener valueEventListener = databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    list.add(comment);  // update comment list
                }
